async function articulo() {
	...
	if (n == 1){
		document.querySelector("#texto").innerHTML += '<div class="enlaces col-4"><a class="' + color + '" href="https://github.com/AndresMartinM" target="_blank" aria-label="Enlace a GitHub">GitHub</a></div>'
	}
	if (n == 3){
		document.querySelector("#texto").innerHTML += '<div class="enlaces col-4"><a class="' + color + '" href="https://www.youtube.com/watch?v=4RZLKAtOIvI" target="_blank" aria-label="Enlace a Video">Video</a></div>'
	}
	if (n == 4){
		document.querySelector("#texto").innerHTML += '<div class="enlaces col-4"><a class="' + color + '" href="https://andresmartin.itch.io/" target="_blank" aria-label="Enlace a itch.io">itch.io</a></div>'
	}
	if (n == 6){
		document.querySelector("#texto").innerHTML += '<div class="enlaces col-4"><a class="' + color + '" href="https://sketchfab.com/andergenio" target="_blank" aria-label="Enlace a Sketchfab">Sketchfab</a></div>'
	}
}
...
async function tarjetamd() {
    ...
    donde.forEach(function (t, i) {
        donde[i].innerHTML += '<a href="articulo.html?nro='+(i+2)+'" ><div class="tarjeta" style="background : url('+ data[i+2].image[0] +'); background-repeat: no-repeat; background-size: 100% auto;"><div class="sombra"></div><h3>' + data[i+2].title + '<p>' + data[i+2].text[0] + '</p></h3><h4>'+data[i+2].year +'</h4></div></a>';
    });
}
